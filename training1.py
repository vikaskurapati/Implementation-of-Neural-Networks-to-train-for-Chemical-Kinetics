...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(nn.Linear(8,10),nn.Sigmoid(),nn.Linear(10,1))
...
class Nodes(Dataset):
    ...
    def __init__(self,train=True, transform=None):
        self.tensors= [torch.Tensor(np.load('input_1_1.npy')),torch.Tensor(np.load('output_H2_1.npy'))]
...
#optimizer = torch.optim.SGD(net.parameters(), lr=1e-3, momentum=0.4)
...
for epoch in range(num_epochs):
    ...
    if (0.5*(train_loss[-1]+test_loss[-1]) < loss_):
        loss_ = 0.5*(train_loss[-1]+test_loss[-1])
        model_scripted = torch.jit.script(net)
        model_scripted.save('model.pt')


plt.plot(train_loss[int(num_epochs/2):], '*', label='Training')
plt.plot(test_loss[int(num_epochs/2):], '*', label='Test')
plt.legend()
plt.show()
'''
model_scripted = torch.jit.script(net)
